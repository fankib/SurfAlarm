										                  ...
										                  m_dAverageFitness(0)
{
	...
	for (int h=0; h<m_iPopSize; ++h)
	{
		...
		for (int j=0; j<m_iChromoLength; ++j)
		{
			...
			// use between 0..1:
			// m_vecPop[i].vecWeights.push_back(0.5);
		...
		for (int i = 0; i < 71; i++) {
			m_vecPop[h].vecWeights.push_back(0.5);
		}
		for (int i = 0; i < 25; i++) {
			m_vecPop[h].vecWeights.push_back(0.25);
		}
		for (int i = 0; i < 25; i++) {
			m_vecPop[h].vecWeights.push_back(1.0);
		}
		for (int i = 0; i < 1; i++) {
			m_vecPop[h].vecWeights.push_back(12.5);
		}
	}
}
...
vector<SGenome> CGenAlg::Epoch(vector<SGenome> &old_pop)
{
  ...
  //wheel sampling will crash
	int copiesElite = 1;
	int numElite = 4;
	if (!(copiesElite * numElite % 2))
	{
		GrabNBest(numElite, copiesElite, vecNewPop);
	}
